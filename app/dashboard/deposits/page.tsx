...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-semibold ${map[status] || "bg-gray-100 text-gray-700"}`}
    >
      {icons[status]} {status}
    </span>
  );
...
export default function DepositsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (wal.data) {
        const activeWallets = wal.data as Wallet[];

        setWallets(activeWallets);

        // Automatically select the first available wallet
        if (activeWallets.length > 0) {
          setForm((f) => ({
            ...f,
            crypto_type: activeWallets[0].crypto_type,
  ...
  const selectedWallet = wallets.find(
    (w) => w.crypto_type === form.crypto_type,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      setError(
        `Amount must be between $${selectedPlan.min_amount} and $${selectedPlan.max_amount} for ${selectedPlan.name}`,
      );
      return;
    }
    setSubmitting(true);
    ...
    const { data: depositData, error: depErr } = await supabase
      .from("deposits")
      .insert({
        user_id: user.id,
        amount,
      ...
      <div className="p-6 space-y-6">
        ...
        {!showForm && (
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-amber-500 text-black font-bold rounded-xl hover:bg-amber-400 transition-all duration-200 shadow-md shadow-amber-200"
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, crypto_type: e.target.value }))
                    }
                  ...
                  >
                    {wallets.map((wallet) => (
                      <option key={wallet.id} value={wallet.crypto_type}>
                        {wallet.label || wallet.crypto_type}
                      </option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 text-sm"
                    placeholder="100.00"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-amber-500" /> Select
                ...
                >
                  ...
                  {plans.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.name} — {p.roi_percent}% ROI / {p.duration_days} days
                      (Min: ${p.min_amount})
                    </option>
                  ))}
                </select>
                {selectedPlan && (
                  <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
                    ...
                    {selectedPlan.roi_percent}% return over{" "}
                    {selectedPlan.duration_days} days. Expected profit on $
                    {form.amount || "0"}:{" "}
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1">
                      <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                        ...
                        {selectedWallet.label || selectedWallet.crypto_type}{" "}
                        Wallet ({selectedWallet.network})
                      </div>
                      <div className="flex items-center gap-3 mb-3">
                        <div className="flex-1 font-mono text-sm text-gray-800 bg-white border border-gray-200 rounded-lg px-3 py-2 break-all">
                      ...
                      <p className="text-xs text-gray-400">
                        Scan the QR code with your wallet app or copy the
                        address above.
                      </p>
                    </div>
                    {qrUrl && (
              ...
              <div>
                ...
                <input
                  ...
                  value={form.txn_hash}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, txn_hash: e.target.value }))
                  }
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 text-sm font-mono"
                  placeholder="0x..."
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    e.target.files?.[0] &&
                    setForm((f) => ({ ...f, screenshot: e.target.files![0] }))
                  }
                />
              </div>

              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                <strong>Important:</strong> Send the exact amount to the wallet
                address above. Deposits are reviewed within 1-24 hours. Include
                your transaction hash for faster processing.
                {selectedPlan && (
                  <>
                    {" "}
                    Your investment will auto-start once the admin approves this
                    deposit.
                  </>
                )}
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-3.5 bg-amber-500 text-black font-bold rounded-xl hover:bg-amber-400 transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  {submitting ? (
                    <div className="w-5 h-5 border-2 border-black/30 border-t-black rounded-full animate-spin" />
                  ) : (
                    <>
                      <ArrowDownCircle className="w-4 h-4" /> Submit Deposit
                    </>
                  )}
                </button>
              </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {deposits.map((d: any) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-sm font-mono text-gray-500 max-w-xs truncate">
                        {d.txn_hash || "—"}
                      </td>
