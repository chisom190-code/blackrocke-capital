...
export default function ReferralsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
     ...
     if (refRes.data) {
  const userIds = refRes.data.map((r) => r.referred_id);
  ...
  const merged = refRes.data.map((r) => ({
    ...r,
  ...
  const copy = (type: "code" | "link") => {
    navigator.clipboard.writeText(
      type === "code" ? referralCode : referralLink,
    );
    setCopied(type);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => copy("code")}
                  className="flex items-center gap-1.5 px-4 py-3 bg-amber-500 text-black rounded-xl font-bold text-sm hover:bg-amber-400 transition-colors"
                >
                  {copied === "code" ? (
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => copy("link")}
                  className="flex items-center gap-1.5 px-4 py-3 bg-white/10 border border-white/20 text-white rounded-xl font-bold text-sm hover:bg-white/20 transition-colors"
                >
                  {copied === "link" ? (
                    <CheckCircle className="w-4 h-4" />
                  ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              },
              {
                step: "02",
                title: "They Register",
                desc: "Your referral signs up using your code and makes their first investment.",
              },
              {
                step: "03",
                title: "Earn 5% Commission",
                desc: "Receive 5% commission on every investment your referral makes, paid instantly.",
              },
            ].map((s, i) => (
              <div key={i} className="flex gap-4">
                <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center font-black text-black text-sm flex-shrink-0">
                  {s.step}
                </div>
                <div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {referrals.map((r: any) => (
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-black text-sm">
                            {r.referred?.full_name || "Anonymous"}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full font-semibold ${r.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}`}
                        >
                          {r.status}
                        </span>
                      </td>
