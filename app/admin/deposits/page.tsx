    ...
    if (search)
      r = r.filter(
        (d: any) =>
          ...
          d.crypto_type.toLowerCase().includes(search.toLowerCase()) ||
          d.txn_hash?.includes(search),
      );
    if (statusFilter !== "all")
      r = r.filter((d: any) => d.status === statusFilter);
    setFiltered(r);
  ...
  const updateStatus = async (
    ...
    try {
      ...
      if (sessionError || !session) {
        ...
        showMsg("You are not authenticated.");
        return;
      }
      ...
      if (!supabaseUrl || !anonKey) {
        showMsg("Supabase configuration error.");
        return;
      }

      const response = await fetch(
        `${supabaseUrl}/functions/v1/handle-deposit`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok || result.error) {
        showMsg(result.error || `Request failed (${response.status})`);
        return;
      }
      ...
      if (depositsError) {
        console.error("Deposit refresh error:", depositsError);
      } else {
        setDeposits(depositsData ?? []);
        setFiltered(depositsData ?? []);
      }
      ...
      showMsg("Failed to contact the deposit service.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminHeader
        ...
        subtitle="Review and approve user deposits"
      />
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            <div className="text-2xl font-black mb-1">{deposits.length}</div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 flex flex-wrap gap-4">
          <div className="flex-1 min-w-48 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-amber-400 text-sm"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-amber-400 bg-white text-sm"
          >
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((d: any) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-xs font-mono text-gray-500 max-w-xs truncate">
                        {d.txn_hash || "—"}
                      </td>
                      <td className="px-5 py-4">
                        {d.screenshot_url ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex items-center gap-1 text-amber-600 text-xs font-semibold hover:underline"
                          >
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full font-semibold ${d.status === "approved" ? "bg-green-100 text-green-700" : d.status === "rejected" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                        >
                          {d.status}
                        </span>
                      </td>
